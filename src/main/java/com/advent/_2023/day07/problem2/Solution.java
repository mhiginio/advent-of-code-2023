...
public class Solution {
    ...
    public static class Hand {
        ...
        public int getType() {
            Collection<Integer> frequencies = processJokers(frequencies()).values();
        ...
        private Map<Character, Integer> processJokers(Map<Character, Integer> frequencies) {
            Integer jokerCount = frequencies.get('J');
            if (jokerCount != null && jokerCount < 5 && jokerCount > 0) {
                frequencies.remove('J');
                Character max = frequencies.entrySet().stream()
                        .max(Map.Entry.comparingByValue())
                        .get().getKey();
                frequencies.computeIfPresent(max, (key, val) -> val + jokerCount);
            }
            return frequencies;
        }
    ...
    public static final class HandComparator implements Comparator<Hand> {
        private static final String SORTED_CARDS = "AKQT98765432J";
